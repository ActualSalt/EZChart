<?php
	...
	//Check 
	$sql = "SELECT * FROM $db_name.Patient WHERE P_ID='$random'";
	$result = $con->query($sql);
	$row = $result->fetch_assoc();
	$P_ID_Check = $row["P_ID"];
	if($P_ID_Check==null){
...
$qry = "INSERT INTO $db_name.Patient (`E_ID`, `P_ID`, `Charter`, `FName`, `LName`, `DOB`, `Sex`, `Height`, `Weight`, `Pic`, `RoomNumber`, `SSN`, `Insurance`, `TimeStamp`) VALUES (NULL, '$Patient_ID', '$username', '$first_name', '$last_name', '$dob', '$sex', '$height', '$weight', '$pic', '$roomNumber', '$SSN', '$Insurance', CURRENT_TIMESTAMP)";
